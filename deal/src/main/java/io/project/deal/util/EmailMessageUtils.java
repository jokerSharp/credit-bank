...
public class EmailMessageUtils {
    ...
    public static String sesCodeMailMessage(String code, String url) {
        String actualMessage = """
                Код для подтверждения вашей заявки %s
                Его необходимо ввести по адресу %s""".formatted(code, url);
        return getTemplateEmailMessage(actualMessage);
    }
    ...
    public static String wrongCodeMailMessage() {
        String actualMessage = "Вы ввели неверный код подтверждения";
        return getTemplateEmailMessage(actualMessage);
    }
